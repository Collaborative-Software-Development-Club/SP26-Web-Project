          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-0 relative z-10 h-full">
            ...
            <div className="md:col-span-5 flex flex-col p-5 gap-3 border-b md:border-b-0 md:border-r border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-black/20 h-full">
              <div className="w-full flex-1 min-h-[300px] md:min-h-0 rounded-2xl overflow-hidden relative shadow-inner bg-zinc-200 dark:bg-zinc-800 group">
                <Image src="/demo/selfie.png" alt="User Avatar" fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover" />
              </div>
              <div className="grid grid-cols-2 gap-2 h-24 shrink-0">
                <div className="rounded-xl bg-zinc-200 dark:bg-zinc-800 relative overflow-hidden flex items-center justify-center text-zinc-400">
                  <Image src="/demo/room1.png" alt="Room 1" fill sizes="(max-width: 768px) 50vw, 16vw" className="object-cover" />
                </div>
                <div className="rounded-xl bg-zinc-200 dark:bg-zinc-800 relative overflow-hidden flex items-center justify-center text-zinc-400">
                  <Image src="/demo/room2.png" alt="Room 2" fill sizes="(max-width: 768px) 50vw, 16vw" className="object-cover" />
                </div>
              </div>
            </div>
                ...
                <div className="grid grid-cols-2 gap-y-5 gap-x-4">
                  <div className="col-span-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {profile.hobbies.map((hobby) => (
                        <span key={hobby} className="px-3 py-1 rounded-full bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 text-xs font-medium border border-zinc-200 dark:border-zinc-700">
                          {hobby}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="col-span-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {profile.preferences.slice(0, isExpanded ? profile.preferences.length : INITIAL_VISIBLE_PREFS).map((pref) => (
                        <div key={pref.name} className="flex items-center gap-2 p-2.5 rounded-xl bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-100 dark:border-zinc-800">
                          ...
                          <div className="min-w-0">
                            <p className="text-[9px] text-zinc-400 uppercase font-semibold truncate">{pref.name}</p>
                            <p className="text-xs font-medium text-zinc-800 dark:text-zinc-200 capitalize truncate">{pref.value}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                    {profile.preferences.length > INITIAL_VISIBLE_PREFS && (
                      <button onClick={() => setIsExpanded(!isExpanded)} className="mt-2 text-xs font-semibold text-indigo-500 hover:text-indigo-600 transition-colors flex items-center gap-1">
                        {isExpanded ? "Show Less" : `+${profile.preferences.length - INITIAL_VISIBLE_PREFS} More`}
                      </button>
                    )}
                  </div>
                </div>
